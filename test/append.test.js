...
describe('SourceMap - Append Mappings', () => {
  ...
  it('Append source map with line offset that overlap original lines', () => {
    ...
    let mapTwo = new SourceMap('/');
    mapTwo.addIndexedMappings([
      {
        source: 'two.js',
        name: 'F',
        original: {
    ...
    console.log(map.getMap().mappings);

    map.addSourceMap(mapTwo, 10);

    let mappings = map.getMap().mappings;

    assert.deepEqual(mappings[0], {
      generated: { line: 6, column: 15 },
      original: { line: 1, column: 0 },
      source: 0,
      name: 0,
    });

    assert.deepEqual(mappings[1], { generated: { line: 12, column: 6 } });

    assert.deepEqual(mappings[2], {
      generated: { line: 12, column: 45 },
      original: { line: 2, column: 5 },
      source: 1,
      name: 1,
    });

    assert.deepEqual(mappings[3], { generated: { line: 14, column: 12 } });
  });
